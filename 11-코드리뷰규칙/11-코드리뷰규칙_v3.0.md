...
Transition ID는 Jira 프로젝트마다 다르므로, GitHub Actions에서 Jira REST API(`/rest/api/2/issue/{issueKey}/transitions`)를 호출하여 동적으로 조회하는 것을 권장한다. 하드코딩된 Transition ID는 프로젝트 변경 시 오작동을 유발할 수 있다.
  ...
  sync-jira:
    ...
    steps:
      ...
      - name: Fetch Transition ID Dynamically
        ...
        run: |
          # Jira REST API로 현재 이슈의 가능한 전환 목록 조회 (동적 조회 권장)
          TRANSITIONS=$(curl -s \
...
| 누락 처리 | 72시간 초과 시 TL이 Jira 이슈에 사유 기록 + 다음 스프린트 내 처리 |
