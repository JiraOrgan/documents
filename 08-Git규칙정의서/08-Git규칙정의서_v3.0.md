...
### 5.3 Hotfix 브랜치 보호 바이패스 정책

운영 장애 등 긴급 상황에서 Hotfix를 신속하게 배포해야 할 경우, 브랜치 보호 규칙을 제한적으로 우회할 수 있다.

#### 바이패스 권한

- `bypass branch protection` 권한은 **PM(Project Manager)** 및 **TL(Tech Lead)** 에게만 부여한다.
- 권한 부여는 GitHub Organization Admin이 수행하며, 역할 변경 시 즉시 재조정한다.

#### 바이패스 조건

| 항목 | 규칙 |
|------|------|
| 적용 브랜치 | `hotfix/*` → `main` 머지에 한정 |
| 최소 승인 수 | 1명 (PM 또는 TL 중 1인 승인으로 머지 허용) |
| CI 빌드 | 바이패스 시에도 CI 빌드 통과 필수 (Status Check 유지) |
| 사후 정식 리뷰 | 머지 후 **72시간 이내** 정식 리뷰 수행 의무 |

#### 사후 의무 절차

```
1. Hotfix 머지 즉시: Slack #hotfix-log 채널에 자동 알림 발송
...
name: Hotfix Merge Notification
...
jobs:
  notify-hotfix:
    ...
    steps:
      - name: Notify Slack #hotfix-log
        uses: slackapi/slack-github-action@v1
        with:
          payload: |
            {
              "text": "[HOTFIX 머지] PR #${{ github.event.pull_request.number }} `${{ github.event.pull_request.title }}`\n머지 수행자: ${{ github.event.pull_request.merged_by.login }}\nPR 링크: ${{ github.event.pull_request.html_url }}\n⚠️ 72시간 이내 정식 리뷰 필수",
              "channel": "#hotfix-log"
            }
        env:
...
> Production 환경에 `latest`, `sha-*`, `-rc.*` 태그 이미지를 배포하는 것을 금지한다. CI/CD 파이프라인에서 태그 형식 검증을 자동화한다.

```yaml
# GitHub Actions: Production 배포 시 태그 형식 검증 예시
- name: Validate Production Image Tag
  run: |
    if [[ ! "${{ env.IMAGE_TAG }}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      echo "ERROR: Production 배포 태그는 v{semver} 형식이어야 합니다. (예: v1.2.3)"
      echo "현재 태그: ${{ env.IMAGE_TAG }}"
      exit 1
    fi
```
...
| RC | `vX.Y.Z-rc.N` | `v1.0.0-rc.1` | 최종 출시 후보 (Staging 배포에 사용) |
