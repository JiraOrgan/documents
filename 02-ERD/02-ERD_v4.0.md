...
erDiagram
    ...
    BOARD {
        ...
        varchar filter_jql "보드 필터 JQL"
        jsonb column_config "컬럼 설정 (JSONB)"
        boolean is_default "기본 보드 여부"
        bigint created_by FK "생성자 ID"
        timestamp created_at "생성일"
        timestamp updated_at "수정일"
    }
    ...
    USER_ACCOUNT {
        ...
        varchar name "이름"
        varchar avatar_url "아바타 URL"
        varchar timezone "타임존"
        varchar locale "로케일"
        boolean is_active "활성 여부"
        int login_fail_count "로그인 실패 횟수"
        timestamp locked_until "계정 잠금 해제 일시"
        timestamp created_at "생성일"
        timestamp updated_at "수정일"
    }
    ...
    COMMENT {
        ...
        text body "댓글 내용"
        enum visibility_type "공개 범위 (PUBLIC, ROLE, GROUP)"
        varchar visibility_value "공개 범위 값 (역할명/그룹명)"
        timestamp created_at "생성일"
        timestamp updated_at "수정일"
    }
    ...
    ISSUE_LINK {
        ...
        enum link_type "관계 (BLOCKS, DUPLICATES, RELATES_TO, CLONES)"
    }
    ...
    AUTOMATION_EXECUTION_LOG {
        ...
        bigint rule_id FK "규칙 ID"
        varchar trigger_event_type "트리거 이벤트 타입"
        bigint trigger_issue_id FK "트리거 이슈 ID (NULL 허용)"
        enum status "실행 결과 (SUCCESS, FAILURE, SKIPPED)"
        int execution_time_ms "실행 시간 (밀리초)"
    ...
    EXTERNAL_INTEGRATION {
        bigint id PK "연동 ID"
        bigint project_id FK "프로젝트 ID"
        enum integration_type "연동 타입 (GITHUB, SLACK, JIRA_CLOUD, CI_CD)"
        jsonb config "설정 (JSONB 암호화)"
        varchar webhook_url "Webhook URL"
        varchar webhook_secret "Webhook 시크릿"
        boolean is_active "활성 여부"
        bigint created_by FK "생성자 ID"
        timestamp created_at "생성일"
        timestamp updated_at "수정일"
    ...
    PROJECT ||--o{ EXTERNAL_INTEGRATION : "외부 연동"
    USER_ACCOUNT ||--o{ BOARD : "생성"
    ...
    USER_ACCOUNT ||--o{ EXTERNAL_INTEGRATION : "생성"
    ISSUE ||--o{ COMMENT : "포함"
    ...
    AUTOMATION_RULE ||--o{ AUTOMATION_EXECUTION_LOG : "실행 로그"
...
> v4.0 변경: board_type 컬럼 제거 (BOARD 테이블로 이관).
...
### 3.2 BOARD (보드) — v4.0 신규 추가

> v4.0 신규 추가. 프로젝트당 복수의 보드를 지원하기 위해 별도 BOARD 테이블을 추가한다. 기존 PROJECT.board_type 컬럼은 본 테이블로 이관되었다.

| 컬럼명 | 타입 | 제약조건 | 기본값 | 설명 |
|--------|------|----------|--------|------|
| id | BIGINT | PK, AUTO_INCREMENT | - | 보드 ID |
...
| filter_jql | VARCHAR(2000) | - | NULL | 보드 이슈 필터 JQL |
| column_config | JSONB | - | NULL | 컬럼 설정 (칸반 컬럼 구성 등) |
| is_default | BOOLEAN | NOT NULL | false | 기본 보드 여부 |
| created_by | BIGINT | FK(USER_ACCOUNT), NOT NULL | - | 생성자 ID |
| created_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 생성일 |
| updated_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 수정일 |

**제약조건**:
- 부분 UNIQUE 인덱스: `uq_board_project_default` (project_id, is_default) WHERE is_default = true — 프로젝트당 기본 보드 1개 제한

**인덱스**:
- `idx_board_project_id` (project_id) — 프로젝트별 보드 조회
- `uq_board_project_default` (project_id, is_default) WHERE is_default = true — 기본 보드 유일성 보장 (부분 인덱스)

```sql
-- PostgreSQL 부분 유니크 인덱스 (기본 보드는 프로젝트당 1개)
CREATE UNIQUE INDEX uq_board_project_default ON board (project_id) WHERE is_default = true;
```

---

### 3.3 USER_ACCOUNT (사용자)

> v2.0 변경: role 컬럼 제거. 프로젝트별 역할은 PROJECT_MEMBER 테이블에서 관리.
> v4.0 변경: avatar_url, timezone, locale, is_active, login_fail_count, locked_until 컬럼 추가. 관리자 초대 방식으로 사용자 등록.
...
| timezone | VARCHAR(50) | NOT NULL | 'Asia/Seoul' | 타임존 |
| locale | VARCHAR(10) | NOT NULL | 'ko_KR' | 로케일 |
| is_active | BOOLEAN | NOT NULL | true | 활성 계정 여부 (비활성화 시 로그인 불가) |
| login_fail_count | INT | NOT NULL | 0 | 연속 로그인 실패 횟수 (5회 초과 시 잠금) |
| locked_until | TIMESTAMP | - | NULL | 계정 잠금 해제 일시 (NULL이면 잠금 없음) |
| created_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 생성일 |
| updated_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 수정일 |
...
- `idx_user_is_active` (is_active) — 활성 사용자 필터

---

### 3.4 PROJECT_MEMBER (프로젝트 멤버)
...
> v4.0 변경: start_date 컬럼 추가 (로드맵/타임라인 기능 지원).
...
| start_date | DATE | - | NULL | 시작일 (로드맵/타임라인 표시용) |
...
- `idx_issue_start_date` (start_date) — 로드맵/타임라인 날짜 범위 조회
...
### 3.8 WORKFLOW_TRANSITION (워크플로우 전환 이력)
...
> v4.0 변경: visibility_type, visibility_value 컬럼 추가 (댓글 공개 범위 제어).
...
| visibility_type | ENUM('PUBLIC','ROLE','GROUP') | NOT NULL | 'PUBLIC' | 공개 범위 타입 |
| visibility_value | VARCHAR(100) | - | NULL | 공개 범위 값 (ROLE이면 역할명, GROUP이면 그룹명, PUBLIC이면 NULL) |
| created_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 생성일 |
| updated_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 수정일 |
...
### 3.10 ISSUE_LINK (이슈 연결)

> v4.0 변경: link_type ENUM에 CLONES 추가.
...
| link_type | ENUM('BLOCKS','DUPLICATES','RELATES_TO','CLONES') | NOT NULL | - | 관계 타입 |
...
### 3.16 ISSUE_COMPONENT (이슈-컴포넌트 N:M)
...
### 3.17 ISSUE_FIX_VERSION (이슈-릴리즈 버전 N:M)
...
### 3.23 AUTOMATION_TRIGGER (자동화 트리거)
...
### 3.24 AUTOMATION_CONDITION (자동화 조건)
...
### 3.26 AUTOMATION_EXECUTION_LOG (자동화 실행 로그) — v4.0 신규 추가

> v4.0 신규 추가. 자동화 규칙의 실행 결과를 기록하여 디버깅 및 모니터링을 지원한다.

| 컬럼명 | 타입 | 제약조건 | 기본값 | 설명 |
|--------|------|----------|--------|------|
| id | BIGINT | PK, AUTO_INCREMENT | - | 실행 로그 ID |
| rule_id | BIGINT | FK(AUTOMATION_RULE), NOT NULL | - | 자동화 규칙 ID |
| trigger_event_type | VARCHAR(100) | NOT NULL | - | 트리거된 이벤트 타입 (예: STATUS_CHANGED) |
| trigger_issue_id | BIGINT | FK(ISSUE), SET NULL | NULL | 트리거를 유발한 이슈 ID (스케줄 트리거인 경우 NULL) |
| status | ENUM('SUCCESS','FAILURE','SKIPPED') | NOT NULL | - | 실행 결과 상태 |
| execution_time_ms | INT | - | NULL | 실행 소요 시간 (밀리초) |
| error_message | TEXT | - | NULL | 실패 시 오류 메시지 |
| retry_count | INT | NOT NULL | 0 | 재시도 횟수 |
| created_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 실행 시각 |

**인덱스**:
- `idx_ael_rule` (rule_id) — 규칙별 실행 로그 조회
- `idx_ael_status` (status) — 실행 결과별 조회
- `idx_ael_rule_created` (rule_id, created_at) — 규칙별 실행 이력 시간순 조회

---

### 3.27 NOTIFICATION_SUBSCRIPTION (알림 구독)
...
| 컬럼명 | 타입 | 제약조건 | 기본값 | 설명 |
|--------|------|----------|--------|------|
| id | BIGINT | PK, AUTO_INCREMENT | - | 연동 ID |
| project_id | BIGINT | FK(PROJECT), NOT NULL | - | 프로젝트 ID |
| integration_type | ENUM('GITHUB','SLACK','JIRA_CLOUD','CI_CD') | NOT NULL | - | 연동 타입 |
| config | JSONB | NOT NULL | - | 연동 설정 (암호화 저장, API 키/토큰 등) |
| webhook_url | VARCHAR(2000) | - | NULL | Webhook 수신 URL |
| webhook_secret | VARCHAR(500) | - | NULL | Webhook HMAC 시크릿 (암호화 저장) |
...
**인덱스**:
- `idx_ext_int_project_type` (project_id, integration_type) — 프로젝트+연동 타입 조회
- `idx_ext_int_active` (is_active) — 활성 연동 필터

---

### 3.30 PLANNING_POKER_SESSION (플래닝 포커 세션)
...
### 3.31 PLANNING_POKER_VOTE (플래닝 포커 투표)
...
### 3.34 SECURITY_LEVEL_PERMISSION (보안 레벨 권한)
...
|------------|------------|---------|-----------|------|
| PROJECT | BOARD | project_id | CASCADE | 프로젝트 삭제 시 보드 삭제 |
...
| PROJECT | EXTERNAL_INTEGRATION | project_id | CASCADE | 프로젝트 삭제 시 외부 연동 설정 삭제 |
| USER_ACCOUNT | BOARD | created_by | RESTRICT | 보드 생성자가 있는 한 사용자 삭제 불가 |
...
| USER_ACCOUNT | EXTERNAL_INTEGRATION | created_by | RESTRICT | 연동 생성자가 있는 한 삭제 불가 |
...
| ISSUE | AUTOMATION_EXECUTION_LOG | trigger_issue_id | SET NULL | 이슈 삭제 시 실행 로그 trigger_issue_id NULL |
...
| AUTOMATION_RULE | AUTOMATION_EXECUTION_LOG | rule_id | CASCADE | 규칙 삭제 시 실행 로그 삭제 |
...
| BOARD | idx_board_project_id | project_id | BTREE | 프로젝트별 보드 조회 |
| BOARD | uq_board_project_default | (project_id) WHERE is_default=true | UNIQUE PARTIAL | 기본 보드 유일성 |
| USER_ACCOUNT | idx_user_email | email | UNIQUE BTREE | 로그인 조회 |
| USER_ACCOUNT | idx_user_name | name | BTREE | 이름 검색 |
| USER_ACCOUNT | idx_user_is_active | is_active | BTREE | 활성 사용자 필터 |
...
| ISSUE | idx_issue_start_date | start_date | BTREE | 로드맵/타임라인 날짜 조회 |
...
| AUTOMATION_EXECUTION_LOG | idx_ael_rule | rule_id | BTREE | 규칙별 실행 로그 |
| AUTOMATION_EXECUTION_LOG | idx_ael_status | status | BTREE | 실행 결과별 조회 |
| AUTOMATION_EXECUTION_LOG | idx_ael_rule_created | (rule_id, created_at) | BTREE | 규칙별 실행 시간순 |
...
| EXTERNAL_INTEGRATION | idx_ext_int_project_type | (project_id, integration_type) | BTREE | 프로젝트+연동 타입 조회 |
| EXTERNAL_INTEGRATION | idx_ext_int_active | is_active | BTREE | 활성 연동 필터 |
...
#### AUTOMATION_EXECUTION_LOG — 월별 범위 파티셔닝 권장

```sql
-- 자동화 실행 로그는 대량 누적 예상. 월별 파티셔닝 권장.
CREATE TABLE automation_execution_log (
    id BIGSERIAL,
    rule_id BIGINT NOT NULL,
    trigger_event_type VARCHAR(100) NOT NULL,
    trigger_issue_id BIGINT,
...
| USER_ACCOUNT | is_active = false | v4.0에서 is_active 컬럼 추가. 탈퇴 처리 시 CASCADE 대신 비활성화 우선. |
...
### 6.4 v3.0 → v4.0 마이그레이션 스크립트 요점

```sql
-- 1. BOARD 테이블 생성 (PROJECT.board_type 이관)
CREATE TABLE board (
    id BIGSERIAL PRIMARY KEY,
    project_id BIGINT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    name VARCHAR(200) NOT NULL,
    board_type VARCHAR(10) NOT NULL DEFAULT 'SCRUM'
        CHECK (board_type IN ('SCRUM', 'KANBAN')),
    filter_jql VARCHAR(2000),
    column_config JSONB,
...
-- 기본 보드 유일성 인덱스 (부분 인덱스)
CREATE UNIQUE INDEX uq_board_project_default ON board (project_id) WHERE is_default = true;
CREATE INDEX idx_board_project_id ON board (project_id);

-- 기존 프로젝트의 board_type을 BOARD 테이블로 이관
INSERT INTO board (project_id, name, board_type, is_default, created_by)
SELECT p.id,
       p.name || ' Board',
       p.board_type,
       true,
...
-- PROJECT 테이블에서 board_type 컬럼 제거
ALTER TABLE project DROP COLUMN board_type;

-- 2. ISSUE 테이블에 start_date 추가
ALTER TABLE issue ADD COLUMN start_date DATE DEFAULT NULL;
CREATE INDEX idx_issue_start_date ON issue (start_date);

-- 3. USER_ACCOUNT 컬럼 보완
ALTER TABLE user_account
    ADD COLUMN avatar_url VARCHAR(500) DEFAULT NULL,
    ADD COLUMN timezone VARCHAR(50) NOT NULL DEFAULT 'Asia/Seoul',
    ADD COLUMN locale VARCHAR(10) NOT NULL DEFAULT 'ko_KR',
...
-- 4. COMMENT 테이블에 visibility 컬럼 추가
ALTER TABLE comment
    ADD COLUMN visibility_type VARCHAR(10) NOT NULL DEFAULT 'PUBLIC'
        CHECK (visibility_type IN ('PUBLIC', 'ROLE', 'GROUP')),
    ADD COLUMN visibility_value VARCHAR(100) DEFAULT NULL;

-- 5. ISSUE_LINK link_type ENUM에 CLONES 추가
-- PostgreSQL에서 ENUM 타입 값 추가
ALTER TYPE issue_link_type ADD VALUE 'CLONES';
-- (ENUM 타입명은 실제 구현에 따라 조정)

-- 6. EXTERNAL_INTEGRATION 테이블 생성
CREATE TABLE external_integration (
    id BIGSERIAL PRIMARY KEY,
    project_id BIGINT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    integration_type VARCHAR(20) NOT NULL
        CHECK (integration_type IN ('GITHUB', 'SLACK', 'JIRA_CLOUD', 'CI_CD')),
...
CREATE INDEX idx_ext_int_project_type ON external_integration (project_id, integration_type);
CREATE INDEX idx_ext_int_active ON external_integration (is_active);

-- 7. AUTOMATION_EXECUTION_LOG 테이블 생성
CREATE TABLE automation_execution_log (
    id BIGSERIAL PRIMARY KEY,
    rule_id BIGINT NOT NULL REFERENCES automation_rule(id) ON DELETE CASCADE,
    trigger_event_type VARCHAR(100) NOT NULL,
    trigger_issue_id BIGINT REFERENCES issue(id) ON DELETE SET NULL,
...
CREATE INDEX idx_ael_rule ON automation_execution_log (rule_id);
CREATE INDEX idx_ael_status ON automation_execution_log (status);
CREATE INDEX idx_ael_rule_created ON automation_execution_log (rule_id, created_at);
```
...
| 구분 | 내용 |
|------|------|
| 테이블 추가 | BOARD, EXTERNAL_INTEGRATION, AUTOMATION_EXECUTION_LOG (3개 추가) |
| 테이블 수정 | PROJECT — board_type 컬럼 제거 (BOARD 테이블로 이관) |
| 테이블 수정 | USER_ACCOUNT — avatar_url, timezone, locale, is_active, login_fail_count, locked_until 컬럼 추가 |
| 테이블 수정 | ISSUE — start_date DATE 컬럼 추가 (로드맵/타임라인 지원) |
| 테이블 수정 | COMMENT — visibility_type, visibility_value 컬럼 추가 (댓글 공개 범위 제어) |
| ENUM 수정 | ISSUE_LINK.link_type — CLONES 값 추가 |
| 의사결정 반영 | 프로젝트당 복수 보드 지원 (BOARD 테이블 분리) |
| 의사결정 반영 | 관리자 초대 방식 사용자 등록 (USER_ACCOUNT.is_active, Admin이 사용자 생성) |
| 의사결정 반영 | Planning Poker — 스프린트 관리 화면 모달, API 지원 |
| 인덱스 | 신규/변경 테이블 인덱스 추가 (BOARD 2개, USER_ACCOUNT 1개, ISSUE 1개, AUTOMATION_EXECUTION_LOG 3개, EXTERNAL_INTEGRATION 2개) |
| 마이그레이션 | v3.0 → v4.0 DDL 스크립트 추가 |
...
| v4.0 | 2026-03-22 | Agent | BOARD 엔티티 추가, USER_ACCOUNT 보완, start_date, 외부연동/자동화실행로그 테이블, COMMENT visibility |
