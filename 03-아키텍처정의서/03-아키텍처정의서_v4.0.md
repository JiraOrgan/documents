...
14. [성능 목표](#14-성능-목표)
15. [CQRS (Command Query Responsibility Segregation) 검토](#15-cqrs-command-query-responsibility-segregation-검토)
16. [API Gateway 레이어](#16-api-gateway-레이어)
17. [모바일(Flutter) 아키텍처](#17-모바일flutter-아키텍처)
18. [변경 이력](#변경-이력)
...
## 15. CQRS (Command Query Responsibility Segregation) 검토
...
Flutter 모바일 앱의 아키텍처 원칙과 핵심 구성 요소를 정의한다.

### 17.1 아키텍처 패턴: Clean Architecture

Flutter 앱은 Clean Architecture를 적용하여 3개 레이어로 구성한다.

```mermaid
flowchart TB
    subgraph Presentation["Presentation 레이어"]
        Widgets["위젯 (UI)"]
    ...
    subgraph Domain["Domain 레이어"]
        UseCases["Use Cases\n(비즈니스 로직)"]
        Entities["Entities\n(Issue, Sprint, User 등)"]
        RepoInterfaces["Repository 인터페이스\n(추상 계층)"]
    end
...
| 레이어 | 역할 | 주요 구성 요소 |
|--------|------|---------------|
| Presentation | UI 렌더링, 사용자 이벤트 처리 | Flutter 위젯, Riverpod Notifier |
| Domain | 비즈니스 규칙, 유스케이스 정의 | UseCase, Entity, Repository 인터페이스 |
| Data | 외부 데이터 소스 접근, 캐싱 | Dio HTTP 클라이언트, Hive/Isar 로컬 DB |

### 17.2 상태 관리: Riverpod

Riverpod를 기본 상태 관리 라이브러리로 사용한다. Bloc 패턴도 복잡한 이벤트 흐름이 필요한 화면에 선택적으로 적용할 수 있다.

| 상황 | 권장 패턴 | 설명 |
|------|-----------|------|
| 단순 서버 데이터 조회 | `AsyncNotifierProvider` | 이슈 목록, 스프린트 조회 등 |
| 복잡한 UI 상태 | `StateNotifierProvider` | 보드 드래그 상태, 필터 상태 등 |
| 글로벌 사용자 정보 | `Provider` | 로그인 사용자 정보, 테마 설정 |
| 복잡한 이벤트 스트림 | Bloc (선택적) | 실시간 알림 스트림, 웹소켓 연동 |

### 17.3 오프라인 캐싱 및 동기화

네트워크 미연결 상태에서도 핵심 기능을 제공하기 위해 로컬 DB를 운영한다.

**로컬 DB 선택 기준**

| DB | 용도 | 특징 |
|----|------|------|
| Hive | 경량 키-값 저장 | 사용자 설정, 토큰, 간단한 캐시 |
| Isar | 구조화된 오프라인 데이터 | 이슈 목록, 스프린트 데이터 오프라인 캐시 |

**오프라인 동기화 전략**

```
[오프라인 쓰기 작업]
1. 사용자 액션 발생 (이슈 상태 변경 등)
2. 로컬 DB에 즉시 반영 (Optimistic Update)
3. 동기화 큐(FIFO)에 작업 추가
4. 네트워크 복구 시 큐 순서대로 서버에 전송
5. 충돌 발생 시: 서버 데이터 우선(Server Wins) 정책 적용
6. 충돌 이력은 로컬 로그에 기록

[충돌 해결 정책]
- 기본: 서버 우선 (Server Wins)
- 예외: 오프라인 중 생성된 신규 이슈는 머지 시도 후 실패 시 사용자에게 알림
```

### 17.4 푸시 알림: FCM (Firebase Cloud Messaging)

| 항목 | 구성 |
|------|------|
| 플랫폼 | FCM (Android + iOS 통합) |
| 패키지 | `firebase_messaging` |
| 알림 채널 | 이벤트 타입별 채널 분리 |
| 백그라운드 처리 | `FirebaseMessaging.onBackgroundMessage` 핸들러 등록 |

**이벤트 타입별 알림 채널**

| 채널 ID | 대상 이벤트 | 중요도 |
|---------|------------|--------|
| `channel_issue_assigned` | 담당자 지정 알림 | 높음 |
| `channel_issue_status` | 상태 전환 알림 | 기본 |
| `channel_comment_mention` | 댓글 @멘션 알림 | 높음 |
| `channel_sprint_event` | 스프린트 시작/완료 | 기본 |
| `channel_wip_exceeded` | WIP 초과 경고 | 낮음 |

### 17.5 토큰 저장: flutter_secure_storage

JWT 토큰 및 민감한 인증 정보는 `flutter_secure_storage` 패키지를 사용하여 플랫폼 보안 저장소에 저장한다.

| 플랫폼 | 저장소 | 비고 |
|--------|--------|------|
| Android | Android Keystore | 하드웨어 지원 암호화 |
| iOS | iOS Keychain | Secure Enclave 활용 |
...
### 17.6 네트워크: Dio + Interceptor

모든 REST API 통신은 Dio 라이브러리를 사용하며, Interceptor로 공통 처리를 적용한다.

```
[Interceptor 체인]
1. AuthInterceptor
   - 요청 헤더에 Access Token 자동 주입
   - 401 응답 시 Refresh Token으로 Access Token 자동 갱신 후 재시도
   - Refresh Token도 만료 시 로그인 화면으로 이동

2. ConnectivityInterceptor
   - 네트워크 미연결 상태 감지
   - 오프라인 상태 시 요청 차단 + 로컬 캐시 응답 반환
   - 네트워크 복구 감지 후 동기화 큐 처리 트리거

3. LoggingInterceptor
   - 개발 환경에서만 요청/응답 로그 출력
   - 프로덕션 환경에서는 비활성화
```
...
final dio = Dio(BaseOptions(
  baseUrl: 'https://api.jira-pm.example.com',
  ...
  ..interceptors.add(AuthInterceptor(storage: secureStorage))
...
| v3.1 | 2026-03-21 | 팀 | 기술 스택 버전 업데이트, Flutter 3.41 / Dart 3.11 Mobile 섹션 신규 추가, 디렉토리 구조에 mobile/ 추가 |
| v4.0 | 2026-03-22 | 팀 | 섹션 번호 중복 수정 (14절 성능목표, 15절 CQRS, 16절 API Gateway), 모바일(Flutter) 아키텍처 17절 신규 추가 (Clean Architecture, Riverpod, 오프라인 캐싱, FCM, flutter_secure_storage, Dio Interceptor) |
