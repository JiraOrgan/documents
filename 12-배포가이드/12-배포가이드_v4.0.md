...
`JIRA_VERSION_ID`는 하드코딩 대신 Jira REST API로 동적 조회합니다. 버전 이름(Git 태그)으로 프로젝트 버전 목록에서 ID를 추출하여 사용합니다.
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Resolve Jira Version ID dynamically
        id: jira-version
        env:
        ...
        run: |
          ...
          if [ -z "$JIRA_VERSION_ID" ]; then
            echo "ERROR: Jira Version 'v${VERSION}' not found in project PROJ"
            exit 1
          fi

          echo "JIRA_VERSION_ID=${JIRA_VERSION_ID}" >> $GITHUB_OUTPUT
          echo "Resolved Jira Version ID: ${JIRA_VERSION_ID} for v${VERSION}"
...
> **참고**: `JIRA_VERSION_ID`를 GitHub Secrets에 하드코딩하지 않으므로 버전이 추가될 때마다 Secrets 업데이트가 불필요합니다. Jira 프로젝트에 버전(Fix Version)이 사전 생성되어 있어야 합니다.
