...
status: "확정"
tags: ["schedule", "gantt", "milestone", "24weeks"]
---

# LearnFlow AI — 개발 일정 v4.0

## 변경 이력

| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| v1.0 | 2025-09-01 | 초기 12주 일정 작성 | AI Assistant |
| v2.0 | 2025-11-15 | 24주 확장, AI 서비스 분리 반영 | AI Assistant |
| v3.0 | 2026-01-20 | Outbox, RAGAS, PII, FinOps Phase 반영 | AI Assistant |
...
| Phase 수 | 6 Phase |
| 마일스톤 | 6개 (M1~M6) |
| 시작일 | 2026-04-07 (Week 1) |
| 종료일 | 2026-09-27 (Week 24) |
...
| Week | Phase | 주요 산출물 | 담당 영역 |
|------|-------|-------------|-----------|
| 1 | P1 | ERD, `users`, `outbox_events`, JWT Filter | Backend |
| 2 | P1 | 강의/섹션/레슨 API, 수강 신청, MinIO 연동 | Backend |
| 3 | P1 | 콘텐츠 에디터, 영상 업로드 API | Backend + Frontend |
| 4 | P1 | React SPA 레이아웃, 강의 목록/상세 UI | Frontend |
| 5 | P2 | 진도 추적 API, `learning_activities`, `LessonCompleted` 이벤트 | Backend |
| 6 | P2 | 퀴즈/과제 API, Appeal API, 채점 UI | Backend + Frontend |
| 7 | P2 | 진단 테스트 API, Bloom's 배분, `confidence_weight` | Backend |
| 8 | P2 | 커뮤니티 게시판 API, 커뮤니티 UI | Backend + Frontend |
| 9 | P3 | `OutboxPublisher`, Kafka Consumer, `dedup_key`, DLQ | Backend (Infra) |
| 10 | P3 | `PiiMaskingService`, `PiiOutputScanner`, AI Gateway | Backend (AI) |
| 11 | P3 | OTel Sampling, Zipkin, Business Context Span | Backend (Infra) |
| 12 | P3 | `CostTracking`, `KillSwitch`, `UnitEconomics`, Semantic Cache | Backend (AI) |
| 13 | P4 | `SemanticChunking`, `chunk_hash`, pgvector, Hybrid Search | Backend (AI) |
| 14 | P4 | CrossEncoder, `QueryRewrite`, `ContextCompression`, Chunk Versioning | Backend (AI) |
| 15 | P4 | AI 튜터 SSE, `LevelingService`, Short/Long-term Memory | Backend (AI) |
| 16 | P4 | `AiQuizGenerator`, `ConfidenceScorer`, `ManualReviewQueue` | Backend (AI) |
| 17 | P5 | `WeaknessDetection`, `ConceptMastery`, 분석 대시보드 | Backend + Frontend |
| 18 | P5 | `RagasEvaluation`, `DeepEvalService`, Importance Sampling 배치 | Backend (AI) |
| 19 | P5 | `AbTestService`, `PromptVersionService`, A/B 관리 UI | Backend + Frontend |
| 20 | P6 | Flutter 앱 (수강·AI 튜터·분석), Riverpod | Mobile |
| 21 | P6 | `NotificationWorker`, Admin 대시보드 UI | Backend + Frontend |
| 22 | P6 | Grafana AI 전용 대시보드 (5개 Panel) | Infra |
| 23 | P6 | Chaos Test 5종, 보안 점검 보고서 | QA + Infra |
| 24 | P6 | Docker Compose 전체 스택, 최종 문서 세트 | 전체 |
