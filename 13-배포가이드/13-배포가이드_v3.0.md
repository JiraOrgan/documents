...
7. [모니터링](#7-모니터링)
8. [Jira Release 자동 연동](#8-jira-release-자동-연동)
...
| 태그 형식 | 예시 | 용도 |
|-----------|------|------|
| `{semver}` | `v1.2.3` | 운영 릴리즈 태그 |
| `{semver}-{git-sha}` | `v1.2.3-a1b2c3d` | 정확한 빌드 추적 |
| `latest` | `latest` | Dev 환경 최신 이미지 |
| `sha-{git-sha}` | `sha-a1b2c3d` | 핫픽스/임시 빌드 |
...
| JIRA_API_TOKEN | Jira REST API 인증 | (AWS Secrets Manager에서 주입) | Y |
...
9. 배포 성공 후 Jira Release 자동 업데이트 및 GitHub Release 자동 게시
...
| 5. Jira Release 되돌리기 | Version API로 Released → Unreleased 전환 | [8.1 참조](#81-version-api-자동-호출) |
| 6. 사후 조치 | 장애 보고서 작성 (RCA) | Jira Bug 이슈로 등록 |
...
모든 롤백 이벤트는 다음 정보를 포함하여 Jira에 Bug 이슈로 등록합니다.
...
- [ ] X-Ray 분산 추적 이상 없음
- [ ] 배포 완료 Slack 공유
- [ ] Jira Release 상태 → Released 자동 전환 확인
...
## 8. Jira Release 자동 연동

배포 성공 후 Jira Version API를 자동 호출하여 릴리즈 상태를 갱신하고, 릴리즈 노트를 자동 생성하여 GitHub Release에 게시합니다.

### 8.1 Version API 자동 호출

배포 파이프라인 마지막 단계에서 Jira REST API를 호출합니다.

```bash
# Jira Version 상태를 Released로 업데이트
...
**전체 이슈**: 4건 | **배포 일시**: 2026-03-21 14:00 KST
```
    ...
    steps:
      ...
      - name: Build, tag, and push Docker image
        env:
          ...
          IMAGE_TAG: ${{ steps.version.outputs.VERSION }}-${{ github.sha }}
        run: |
          docker build -t $ECR_REGISTRY/$ECR_REPOSITORY:$IMAGE_TAG .
          docker tag $ECR_REGISTRY/$ECR_REPOSITORY:$IMAGE_TAG \
                     $ECR_REGISTRY/$ECR_REPOSITORY:${{ steps.version.outputs.VERSION }}
          docker push $ECR_REGISTRY/$ECR_REPOSITORY:$IMAGE_TAG
      ...
      - name: Scan Docker image with Trivy
        ...
        with:
          image-ref: ${{ steps.login-ecr.outputs.registry }}/jira-pm-api:${{ steps.version.outputs.VERSION }}
          format: 'sarif'
          exit-code: '1'
      ...
      - name: Update Jira Release status
        env:
          ...
          JIRA_VERSION_ID: ${{ secrets.JIRA_VERSION_ID }}
        run: |
          curl -X PUT \
            "https://${{ secrets.JIRA_DOMAIN }}/rest/api/3/version/${JIRA_VERSION_ID}" \
            -H "Authorization: Bearer ${JIRA_API_TOKEN}" \
            -H "Content-Type: application/json" \
...
```bash
# 1. 복원 시점 결정 (장애 발생 직전)
RESTORE_TIME="2026-03-21T09:55:00Z"
...
훈련 결과는 Jira Task 이슈로 등록하고 RTO/RPO 달성 여부를 기록합니다.
...
| `jira-pm-{env}/jira-api-token` | Jira REST API 토큰 | 수동 (만료 시) |
